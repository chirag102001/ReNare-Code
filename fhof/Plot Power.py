...
for i in range(1, 7):
    ...
    # Plot the power vs. time for Link 1 using Plotly
    fig_power_link_1 = px.line(prepositioning_rows, x='time', y=f'power_link_{i}',
                               labels={'time': 'Time', f'power_link_{i}': f'Power - Link {i}'},
                               title=f'Power vs Time - Link {i} (Prepositioning)')

    # Show the Plotly figure for Power vs Time
    fig_power_link_1.show()