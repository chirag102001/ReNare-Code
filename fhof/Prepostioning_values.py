...
# Create an empty DataFrame to store individual link energies
link_energy_df = pd.DataFrame(index=range(1, 7), columns=['Total_Kinetic_Energy'])
...
for i in range(1, 7):
    ...
    absolute_power = prepositioning_rows[f'kinetic_energy_link_{i}'].diff(1).abs()
    total_kinetic_energy = absolute_power.sum() * prepositioning_rows["time"].mean()

    # Store the total kinetic energy for the current link in the DataFrame
    link_energy_df.loc[i, 'Total_Kinetic_Energy'] = total_kinetic_energy

    # Display the total kinetic energy for the current link
    print(f"Total Kinetic Energy (Link {i} - Prepositioning):", total_kinetic_energy)

# Calculate and display the sum of kinetic energy across all links
total_kinetic_energy_sum = link_energy_df['Total_Kinetic_Energy'].sum()
print("Sum of Total Kinetic Energy across all links:", total_kinetic_energy_sum)
